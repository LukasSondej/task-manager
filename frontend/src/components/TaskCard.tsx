...
export const TaskCard = ({ task, onEditClick }: PropsTaskCard) => {
  ...
  return (
    <Card className="bg-white border border-slate-200 shadow-sm hover:shadow-md hover:-translate-y-1 transition-all duration-200 rounded-xl flex flex-col h-full">
      <CardHeader>
        <CardTitle className="text-lg font-bold text-slate-900">{task.title}</CardTitle>
        <span
          className={`${colorsTailwind(task.status)} inline-block rounded-full px-2 py-1 text-xs`}
        >
          Status: {task.status}
        </span>
      </CardHeader>
